...
const { rouge1 } = require('../src/services/metrics/logmark/guideMetrics.js');
...
data.forEach((row, index) => {
    if (row['Type of issue'] === 'Hallucination') {
        ...
        hallucinationScores.push({ row: index + 2, score }); // Excel rows are 1-indexed, with header at 1. So data row 0 is Excel row 2.
    }
});
