...
export function buildFairnessInputs(normalizedRows: NormalizedRow[], ctx: any, protectedColumn: string): BuilderResult {
  const hasColumn = normalizedRows.some(r => r.raw[protectedColumn] !== undefined);
  if (!hasColumn) {
    return { status: 'not_available', reason: `requires ${protectedColumn} column to be present` };
  }
  ...
  if (groups.size < 2) {
    return { status: 'not_available', reason: `only one group present for '${protectedColumn}' in this batch — cannot compare` };
  }
  ...
  if (!privilegedGroup || !unprivilegedGroup) {
      return { status: 'not_available', reason: `could not determine privileged/unprivileged groups for ${protectedColumn}` };
  }
  ...
  if (privTotal < 10 || unprivTotal < 10) {
    details.warning = `Low sample size (Privileged: ${privTotal}, Unprivileged: ${unprivTotal}). Results may not be statistically significant.`;
  }
