...
export function buildFairnessInputs(normalizedRows: NormalizedRow[]): BuilderResult {
  const inputs: any[] = [];
  for (const row of normalizedRows) {
    if (row.raw.protected_group !== undefined && row.raw.outcome !== undefined) {
      inputs.push(row.raw);
    }
  }
  
  if (inputs.length === 0) {
    return { status: 'not_available', reason: 'Requires protected group and outcome columns.' };
  }
...
export function buildReasoningInputs(normalizedRows: NormalizedRow[]): BuilderResult {
  ...
  for (const row of normalizedRows) {
    if (row.raw.llm_steps && Array.isArray(row.raw.llm_steps) && row.raw.gold_steps && Array.isArray(row.raw.gold_steps)) {
      llmSteps.push(row.raw.llm_steps);
      goldSteps.push(row.raw.gold_steps);
    }
  }

  if (llmSteps.length === 0) {
    return { status: 'not_available', reason: 'Missing explicit step-by-step reasoning columns (llm_steps, gold_steps).' };
  }

  return { status: 'available', inputs: { llmSteps, goldSteps } };
}
